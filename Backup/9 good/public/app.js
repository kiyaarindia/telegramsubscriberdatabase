...
if (window.location.pathname.includes('dashboard.html')) {
    ...
    function renderAnalytics() {
        const headerEl = document.getElementById('analyticsTableHeader');
        const bodyEl = document.getElementById('analyticsTableBody');
        const analyticsChannelCountEl = document.getElementById('analyticsChannelCount');
        if (!headerEl || !bodyEl) return;

        analyticsCountEl.textContent = analyticsHistory.length;
        if (analyticsChannelCountEl) analyticsChannelCountEl.textContent = channels.length;

        if (analyticsHistory.length === 0) {
            headerEl.innerHTML = '';
            bodyEl.innerHTML = '<tr><td colspan="4" style="text-align:center; padding:2rem; color:var(--text-secondary);">No history recorded yet. Wait for hourly update or click "Record Now".</td></tr>';
            return;
        }

        // 1. Build Header
        // Sort channels: Our -> Relative -> Competitor, then by subscribers desc
        ...
        const currentChannels = [...channels].sort((a, b) => {
            const orderA = ownershipOrder[a.ownership] || 99;
            const orderB = ownershipOrder[b.ownership] || 99;
            if (orderA !== orderB) return orderA - orderB;

            // Secondary sort: Subscribers (descending)
        ...
        // Calculate Ranks based on current subscribers
        const rankedChannels = [...channels].sort((a, b) => parseSubscribers(b.subscribers) - parseSubscribers(a.subscribers));
        const rankMap = {};
        rankedChannels.forEach((c, idx) => {
            rankMap[c.username] = idx + 1;
        });

        let headerHtml = `
            <tr>
                <th rowspan="2" class="sticky-col col-date">Date</th>
                <th rowspan="2" class="sticky-col col-day">Day</th>
                <th rowspan="2" class="sticky-col col-time">Time</th>
        `;

        currentChannels.forEach(c => {
            headerHtml += `<th class="channel-col" title="${c.username}">${c.name}</th>`;
        });
        headerHtml += '</tr><tr>';

        // Second row for Ranks
        currentChannels.forEach(c => {
            headerHtml += `<th class="rank-col">Rank ${rankMap[c.username]}</th>`;
        });
        headerHtml += '</tr>';
        ...
        analyticsHistory.forEach(record => {
            ...
            const row = document.createElement('tr');
            let rowHtml = `
                <td class="sticky-col col-date">${dateStr}</td>
                <td class="sticky-col col-day">${dayStr}</td>
                <td class="sticky-col col-time">${timeStr}</td>
            `;

            currentChannels.forEach(c => {
                const detail = record.details.find(d => d.username === c.username);
                const subs = detail ? detail.subscribers.toLocaleString() : '-';
                rowHtml += `<td class="subs-cell">${subs}</td>`;
            });
    ...
    // Analytics Search & Auto-scroll
    const analyticsSearchInput = document.getElementById('analyticsSearchInput');
    const analyticsTableWrapper = document.getElementById('analyticsTableWrapper');

    if (analyticsSearchInput && analyticsTableWrapper) {
        analyticsSearchInput.addEventListener('input', (e) => {
            const query = e.target.value.toLowerCase().trim();
            if (!query) return;

            const headerCells = document.querySelectorAll('#analyticsTableHeader th.channel-col');
            let targetCell = null;
            ...
            if (targetCell) {
                const wrapperRect = analyticsTableWrapper.getBoundingClientRect();
                const cellRect = targetCell.getBoundingClientRect();
                // 320px is the combined width of the 3 sticky columns (Date, Day, Time)
                const scrollLeft = analyticsTableWrapper.scrollLeft + (cellRect.left - wrapperRect.left) - 320;

                analyticsTableWrapper.scrollTo({
                    left: scrollLeft,
                    behavior: 'smooth'
                });
            }
